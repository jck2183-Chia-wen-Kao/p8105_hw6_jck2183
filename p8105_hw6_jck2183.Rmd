...
birth_df %>% 
    ...
    geom_smooth(method = 'lm')
```
After examining each variable and the birth weight, I decided to keep bhead, blength, gaweeks, momage, smoken, and the interaction between gaweeks and bhead, blenghth, momage, and smoken respectively.
...
```
After comparing the p-value, we only include the predictors with significant p-values. 
